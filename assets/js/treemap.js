(($) => {
    ...
    function TreeMap(container, options) {
        ...
        this.rectangle = new Rectangle(container.offset().left, container.offset().top, container.width(), container.height());
     ...
     * @return {void} This function does not return a value.
     */
    TreeMap.prototype.modal = function ({ data, clientY, clientX }) {
        const { label, summary } = data;
        const { description, review, top, tags } = summary;
        ...
        if (description) {
            ...
            const body = $("<div>")
                .addClass("treemap-modal-body")
                .append(
                    $("<p>").addClass("treemap-modal-title").text(label),
                    $("<p>").addClass("treemap-modal-description").text(description),
                ).appendTo(modal);

            if (this.review?.show) {
                const textReview = review
                    ? `${review} (${top}) avaliações`
                    : "Nenhuma avaliação disponível";

                const reviewContainer = $("<p>")
                    .addClass("treemap-review")
                    .css({
                    ...
                    tags.forEach((tag) => {
                        tagsContainer.append(
                            $("<span>")
                                ...
                                .css({
                                    background: this.tags.background,
     ...
     */
    TreeMap.prototype.sum = (nodes) => nodes.reduce((accumulator, node) => accumulator + node.value, 0);
